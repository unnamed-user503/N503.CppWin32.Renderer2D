...
extern "C"
{
    ...
    __declspec(dllexport) n503_renderer2d_sprite_h n503_renderer2d_sprite_create(const char* path, uint32_t left, uint32_t top, uint32_t right, uint32_t bottom);

    __declspec(dllexport) int n503_renderer2d_sprite_destroy(n503_renderer2d_sprite_h instance);

    __declspec(dllexport) int n503_renderer2d_sprite_set_transform(n503_renderer2d_sprite_h instance, float x, float y, float scaleX, float scaleY, float rotation);

    __declspec(dllexport) int n503_renderer2d_sprite_set_color(n503_renderer2d_sprite_h instance, float r, float g, float b, float a);

    __declspec(dllexport) int n503_renderer2d_sprite_set_render_group(n503_renderer2d_sprite_h instance, uint32_t group);

    __declspec(dllexport) int n503_renderer2d_sprite_set_visible(n503_renderer2d_sprite_h instance, uint32_t visible);


    typedef struct n503_renderer2d_sprite_group_t* n503_renderer2d_sprite_group_h;
    typedef uint32_t (*n503_renderer2d_sprite_group_set_transform_delegate_t)(uint64_t index, float* x, float* y, float* scaleX, float* scaleY, float* rotation, void* user_data);
    typedef uint32_t (*n503_renderer2d_sprite_group_set_color_delegate_t)(uint64_t index, float* r, float* g, float* b, float* a, void* user_data);

    __declspec(dllexport) n503_renderer2d_sprite_group_h n503_renderer2d_sprite_group_create(const char* path, uint32_t count, uint32_t left, uint32_t top, uint32_t right, uint32_t bottom);
