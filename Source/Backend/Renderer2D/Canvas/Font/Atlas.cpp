...
namespace N503::Renderer2D::Canvas::Font
{
    namespace
    {
        ...
        static constexpr std::u32string_view Katakana = U"ァアィイゥウェエォオカガキギクグケゲコゴサザシジスズセゼソゾ"
                                                        ...
                                                        U"ーヽヾ";
        // カタカナ (U+30A1–U+30F6) + 長音符
        static constexpr std::u32string_view AlphabetJP = U"ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ"
        ...
        static constexpr std::u32string_view JoyoKanji = U"亜哀挨愛曖悪握圧扱宛嵐安朝暗以衣位囲医依委威為畏胃尉異移萎偉椅彙意違維慰遺緯域育壱逸茨芋引印因咽姻員院淫陰飲隠韻右宇羽雨唄鬱畝浦運雲"
        ...
        static auto BuildFullCharset() -> std::u32string
        {
            ...
            result.reserve(BasicLatin.size() + Hiragana.size() + Katakana.size() + JapaneseSymbols.size() + AlphabetJP.size() + JoyoKanji.size());
            result += BasicLatin;
            result += Hiragana;
            result += Katakana;
    ...
    Atlas::Atlas(ID2D1DeviceContext3* deviceContext, IDWriteFactory3* dwriteFactory, IDWriteFontFace3* fontFace, float emSize, std::u32string_view charset)
    {
        ...
        for (const char32_t codePoint : m_Charset)
        {
            UINT32 ucp        = static_cast<UINT32>(codePoint);
            UINT16 glyphIndex = 0;
            ...
            const float advanceWidth = glyphMetrics.advanceWidth * scale;
            const float lBearing     = glyphMetrics.leftSideBearing * scale;
            const float rBearing     = glyphMetrics.rightSideBearing * scale;
            const float bearingY     = static_cast<float>(fontMetrics.ascent) * scale;

            const float inkW  = advanceWidth - lBearing - rBearing;
            const float cellW = std::max(inkW, 1.0f);
            const float cellH = std::ceil(static_cast<float>(fontMetrics.ascent + fontMetrics.descent) * scale);

            if (cursorX + inkW + 2.0f > static_cast<float>(AtlasWidth))
            {
                cursorX    = 1.0f;
            ...
            const D2D1_POINT_2F baseline{ cursorX - lBearing, cursorY + bearingY };
            deviceContext->DrawGlyphRun(baseline, &run, brush.get());

            m_Glyphs[codePoint] = Glyph{
                .SourceRect   = D2D1::RectU(static_cast<UINT32>(std::floor(cursorX)), static_cast<UINT32>(std::floor(cursorY)), static_cast<UINT32>(std::ceil(cursorX + cellW)), static_cast<UINT32>(std::ceil(cursorY + cellH))),
    ...
    auto Font::Atlas::GetGlyph(char32_t codePoint) const -> const Glyph*
    {
